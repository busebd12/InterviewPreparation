...
bool hasCycle(const std::vector<std::vector<int>> & graph, std::vector<int> & visited, std::vector<int> & result, int vertex)
{
    if(visited[vertex])
    {
        if(visited[vertex]==1)
    ...
    for(const auto & neighbour : graph[vertex])
    {
        if(hasCycle(graph, visited, result, neighbour))
        {
            return true;
        }
    }
...
std::vector<int> findOrder(int numCourses, std::vector<std::vector<int>> & prerequisites)
{
    ...
    for(auto vertex=0;vertex<numCourses;++vertex)
    {
        if(!visited[vertex])
        {
            if(hasCycle(graph, visited, result, vertex))
            {
                break;
            }
        }
